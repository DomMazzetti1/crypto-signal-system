...
export async function GET() {
  ...
  await runWithConcurrency(symbols, MAX_CONCURRENT, async (symbol) => {
    ...
    for (let i = WARMUP_BARS; i < candles1h.length - FORWARD_BARS; i++) {
      ...
      // Detect signals (exclude SQ_LONG for optimized variant)
      const signals = detectSignals(symbol, indicators).filter(s => s.type !== "SQ_LONG");
  ...
  console.log("[backtest/optimized] Complete:", JSON.stringify(summary));
