...
const SETUP_TYPE = "SQ_SHORT_TRIGGER_SHADOW";
...
export async function GET() {
  ...
  // relaxed_pass = true means state mode fired
  // baseline_pass = true AND relaxed_pass = false means only event mode fired
  const statePass = graded.filter((r) => r.relaxed_pass);
  const eventOnly = graded.filter((r) => r.baseline_pass && !r.relaxed_pass);
  ...
  const stateStats = stats(statePass);
  const eventOnlyStats = stats(eventOnly);

  let decision: string;
  if (graded.length < MIN_GRADED_FOR_DECISION || stateStats.count === 0) {
    decision = "insufficient data";
  } else if (eventOnlyStats.count === 0) {
    decision = "state better so far";
  } else {
    const rDiff = stateStats.avg_r - eventOnlyStats.avg_r;
    const wrDiff = stateStats.win_rate_tp1 - eventOnlyStats.win_rate_tp1;
    if (rDiff > 0.1 && wrDiff >= -0.05) decision = "state better so far";
    else if (rDiff < -0.1 && wrDiff <= 0.05) decision = "event better so far";
    else decision = "roughly equal";
  }

  // Count shadow_only = state fires but event doesn't (new signals from state mode)
  const stateOnlyTotal = rows.filter((r) => r.shadow_only).length;
  const stateOnlyGraded = graded.filter((r) => r.shadow_only).length;
  ...
  return NextResponse.json({
    ...
    state_only_total: stateOnlyTotal,
    state_only_graded: stateOnlyGraded,
    state_pass: stateStats,
    event_only: eventOnlyStats,
    decision,
