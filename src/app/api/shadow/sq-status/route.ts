...
export async function GET() {
  ...
  // Fetch all SQ shadow rows (graded + ungraded)
  const { data: rows, error } = await supabase
    .from("shadow_signals")
    .select("id, symbol, candle_time, adx_1h, rsi, close_price, atr_1h, baseline_pass, relaxed_pass, shadow_only, grade_status, outcome_r, hit_tp1, hit_tp2, hit_tp3, hit_sl, max_favorable, max_adverse, bars_to_resolution, graded_at")
    .eq("setup_type", SETUP_TYPE)
    .order("candle_time", { ascending: false });

  if (error) {
    return NextResponse.json({ error: "Failed to fetch SQ shadow data", detail: error.message }, { status: 500 });
  }
  ...
  // Split graded rows by variant
  // relaxed_pass = true means ADX < 15 (strict threshold passed)
  // baseline_pass = true AND relaxed_pass = false means ADX 15-30 (only baseline passed)
  ...
  const strictStats = stats(strictPass);
  const baselineOnlyStats = stats(baselineOnly);

  // Decision logic
  let decision: string;
  if (graded.length < MIN_GRADED_FOR_DECISION) {
    decision = "insufficient data";
  } else if (strictStats.count === 0) {
    decision = "insufficient data";
  } else if (baselineOnlyStats.count === 0) {
    decision = "strict better so far";
  } else {
    const rDiff = strictStats.avg_r - baselineOnlyStats.avg_r;
    const wrDiff = strictStats.win_rate_tp1 - baselineOnlyStats.win_rate_tp1;
    if (rDiff > 0.1 && wrDiff >= -0.05) {
      decision = "strict better so far";
    } else if (rDiff < -0.1 && wrDiff <= 0.05) {
      decision = "baseline better so far";
    } else {
      decision = "roughly equal";
    }
  }
